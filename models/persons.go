...
var PersonWhere = struct {
	...
	Emailaddress: whereHelpernull_String{field: "\"persons\".\"emailaddress\""},
}
...
type personR struct {
	Stamps StampSlice `boil:"Stamps" json:"Stamps" toml:"Stamps" yaml:"Stamps"`
}
...
func (r *personR) GetStamps() StampSlice {
	if r == nil {
		return nil
	}
	return r.Stamps
...
// Stamps retrieves all the stamp's Stamps with an executor.
func (o *Person) Stamps(mods ...qm.QueryMod) stampQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"stamps\".\"person_id\"=?", o.ID),
	)

	return Stamps(queryMods...)
}

// LoadStamps allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (personL) LoadStamps(ctx context.Context, e boil.ContextExecutor, singular bool, maybePerson interface{}, mods queries.Applicator) error {
	var slice []*Person
	var object *Person
	...
	query := NewQuery(
		qm.From(`stamps`),
		qm.WhereIn(`stamps.person_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load stamps")
	}

	var resultSlice []*Stamp
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice stamps")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on stamps")
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.PersonID) {
				local.R.Stamps = append(local.R.Stamps, foreign)
				if foreign.R == nil {
					foreign.R = &stampR{}
				}
...
// AddStamps adds the given related objects to the existing relationships
// of the person, optionally inserting them as new records.
// Appends related to o.R.Stamps.
// Sets related.R.Person appropriately.
func (o *Person) AddStamps(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Stamp) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.Stamps = append(o.R.Stamps, related...)
	}
...
// SetStamps removes all previously related items of the
// person replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Person's Stamps accordingly.
...
func (o *Person) SetStamps(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Stamp) error {
	...
	return o.AddStamps(ctx, exec, insert, related...)
}

// RemoveStamps relationships from objects passed in.
// Removes related items from R.Stamps (uses pointer comparison, removal does not keep order)
// Sets related.R.Person.
func (o *Person) RemoveStamps(ctx context.Context, exec boil.ContextExecutor, related ...*Stamp) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.Stamps {
			...
			ln := len(o.R.Stamps)
			if ln > 1 && i < ln-1 {
				o.R.Stamps[i] = o.R.Stamps[ln-1]
			}
			o.R.Stamps = o.R.Stamps[:ln-1]
			break
		}
	}
...
func Persons(mods ...qm.QueryMod) personQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"persons\".*"})
	}
